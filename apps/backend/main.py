...
async def startup_event():
    ...
    # Patch FastF1 schedule loading to use local files
    _patch_fastf1_schedule()


def _patch_fastf1_schedule():
    """
    Monkey-patch FastF1 to load schedule from local JSON files
    instead of fetching from GitHub. This allows Docker deployment
    ...
    schedules_dir = Path(__file__).resolve().parents[2] / "data" / "schedules"

    if not schedules_dir.exists():
        logger.warning("Schedule directory not found — FastF1 will use network")
        return

    original_get_schedule = events.get_event_schedule

    def patched_get_schedule(year, **kwargs):
        schedule_file = schedules_dir / f"schedule_{year}.json"
        if schedule_file.exists():
            logger.info(f"Loading schedule from local file: {schedule_file}")
            try:
                import fastf1.events

                data = json.loads(schedule_file.read_text())
                schedule = fastf1.events.EventSchedule(data)
                return schedule
            except Exception as e:
                logger.warning(f"Local schedule failed: {e} — falling back to network")
        return original_get_schedule(year, **kwargs)

    events.get_event_schedule = patched_get_schedule
    logger.info("FastF1 schedule patched to use local files")
